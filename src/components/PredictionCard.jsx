...
function PredictionCard(props) {
    ...
    const { selectedAthlete, meetData, predictionModel, getPrediction, predictionType } = props;

    return (
        <Box m={2} mx="auto" p={4} backgroundColor="gray.100" rounded="md" boxShadow="md" maxWidth="75%">
            {
                (predictionType == "avg2race") ? (
                    <>
                    ...
                    <>
                        <Box textAlign="center" m={4}>
                            <Text fontSize="xl">{selectedAthlete.name}'s performance at {meetData.name} makes them capable of an SR of</Text>
                            <Text fontSize="3xl">{secondsToReadable(getPrediction())}</Text>
                        </Box>
                        <Flex flexDirection="row" justifyContent="space-between">
                            <Text fontSize="md">Model: {predictionModel.string}</Text>
